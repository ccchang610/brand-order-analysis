...
function filteredRows(){return stores.filter(store=>{if(state.region!==T.all&&store.regionGroup!==state.region)return false;if(state.city!=="all"&&store.city!==state.city)return false;if(state.system!=="all"&&!storeHasSystem(store,state.system))return false;if(state.gmb==="confirmed"&&!store.hasGmbOrderingSystem)return false;if(state.gmb==="gap"&&store.hasGmbOrderingSystem)return false;if(state.gmb==="no_gmb_found"&&store.sourceCoverage.gmbFound)return false;if(state.q&&!`${store.storeName} ${store.address} ${store.city} ${store.district}`.toLowerCase().includes(state.q))return false;return true;});}

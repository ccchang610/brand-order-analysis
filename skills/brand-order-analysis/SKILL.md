...
Use this skill to create a brand ordering-system overview. The analysis should show how many stores the brand has, where stores are distributed, which ordering systems are used overall, which ordering systems appear on GMB, and which stores still need manual review.
...
3. Which ordering systems appear on GMB, and where are GMB coverage gaps?
...
7. Compute two separate ordering views:
   ...
   - GMB-only ordering systems
8. Calculate adoption rates using official store count as the denominator.
9. Treat missing or blocked GMB evidence as a coverage gap, not as proof that the store has no ordering system.
10. Generate `data/stores.json`, `data/summary.json`, and optionally `data/stores.csv`.
11. If the user asks for an HTML output, build a dashboard-style report with store overview, all-source ordering overview, GMB ordering overview, comparison table, and store details.
...
- Count a provider as `sourceType: gmb` only when it is read from the Google Business Profile blue online-order button flow, such as `????`, `????`, or `????`, after opening the pickup or delivery panel. Do not infer GMB providers from official ordering links, marketplace links, embedded Maps links, or search results.
- Preserve evidence URLs for ordering-system claims when possible.
- Do not merge all sources into one untraceable provider list; keep all-source and GMB-only views separate.
- Do not infer unavailable dynamic GMB order buttons. Mark them as `no_gmb_order_button`, `unavailable_or_blocked`, or `needs_manual_review`. If Google blocks a re-check but prior confirmed blue-button evidence exists, preserve the confirmed evidence and note the block.
- Keep uncertain stores in the dataset instead of deleting them.

## Google Order Audit Rule

For Google / GMB ordering, always separate entry coverage from provider evidence.
...
- GMB provider evidence means provider names read only after opening that blue order flow and inspecting the pickup or delivery panel.
...
- A `nidin.shop` or `order.nidin.shop` row can count as `Nidin` only when it is a visible provider row inside the opened Google Order panel. The same Nidin URL in an official website link, organic Google result, Maps website row, or background page text must not count as GMB provider evidence.
- Never mark a store as a GMB ordering gap from a fast DOM check alone. If a quick check finds no button, run a human-paced browser re-check: open the GMB or Google result, wait for rendering, move/scroll naturally, click the blue order entry if present, then inspect the panel.
- For `button_confirmed_provider_pending` and blocked Google Order checks, run bounded multi-attempt re-checks before finalizing the status. Default to at least 3 attempts per store when time permits, using Taiwan locale/timezone for Taiwan brands, natural pauses, and both a Google search business-card check and known GMB/Maps URLs. Check Google Search first because the blue ordering buttons may appear on the business card even when the Maps place page only shows a website row. Stop early as soon as provider names are confirmed.
...
- Only create `sourceType: gmb` provider claims after provider names are visible inside the opened Google Order panel and the associated pickup/delivery mode is active. Do not backfill GMB providers from official links, marketplace links, Google snippets, review widgets, or full-page text outside the panel.
...
- `data/summary.json`: overall counts, region/city counts, all-source system counts, GMB system counts, adoption rates, and coverage gaps.
...
3. GMB ordering overview: GMB-found count, GMB ordering-system count, GMB adoption rate, GMB coverage gap, GMB-only provider chart, and region matrix.
4. All-source vs GMB comparison: system name, all-source count/rate, GMB count/rate, and gap.
5. Store details: searchable and filterable table with store, city, region group, address, official source, GMB status, all-source systems, GMB systems, evidence links, and review status.
...
The report filter should support ?? -> region group -> city/county. The same active filter must update KPI cards, map counts, charts, comparison table, and store details.
...
- Confirm GMB adoption rate equals stores with GMB ordering systems divided by official store count.
- Confirm GMB missing or blocked stores are counted as coverage gaps, not as non-adoption.
...
- If an HTML report is built, verify that ??/region/city filters update KPI cards, map, charts, comparison table, and store details together.

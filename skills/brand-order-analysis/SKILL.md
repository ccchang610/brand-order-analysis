...
- Never mark a store as a Google Order gap from a fast DOM check alone. If a quick check finds no button, run a human-paced browser re-check: open the GMB or Google result, wait for rendering, move/scroll naturally, click the blue order entry if present, then inspect the panel.

const T={all:"\u5168\u53f0",north:"\u5317\u90e8",central:"\u4e2d\u90e8",south:"\u5357\u90e8",east:"\u6771\u90e8",islands:"\u96e2\u5cf6",allCities:"\u5168\u90e8\u57ce\u5e02",allSystems:"\u5168\u90e8\u7cfb\u7d71",updated:"\u66f4\u65b0",noData:"\u6c92\u6709\u53ef\u89e3\u6790\u8cc7\u6599",officialStores:"\u5b98\u65b9\u9580\u5e02",currentFilter:"\u76ee\u524d\u7be9\u9078\u7bc4\u570d",gmbMapsFound:"GMB/Maps \u627e\u5230",googleFound:"Google \u627e\u5230",thirdParty:"\u5e73\u53f0/\u7b2c\u4e09\u65b9\u4f86\u6e90",gmbGap:"\u672a\u627e\u5230 Google Order \u5165\u53e3",gmbNotFound:"GMB/Maps \u672a\u627e\u5230",mapGap:"\u5b98\u65b9\u6709\u9580\u5e02\u4f46\u672a\u627e\u5230 GMB",notNoAdoption:"\u5df2\u9010\u5e97\u78ba\u8a8d GMB\uff1b\u672a\u898b Google Order \u85cd\u8272\u5165\u53e3",anySystem:"\u6709\u4efb\u4e00\u7cfb\u7d71",pickupStores:"\u81ea\u53d6\u9580\u5e02",deliveryStores:"\u5916\u9001\u9580\u5e02",unknownStores:"\u672a\u77e5\u9580\u5e02",noEvidence:"\u672a\u898b\u5b98\u65b9/\u5e73\u53f0/Google Order \u4f9b\u61c9\u5546\u8b49\u64da",mainSystems:"\u4e3b\u8981\u7cfb\u7d71\u6578",allSources:"\u5168\u4f86\u6e90",gmbHasSystem:"Google Order \u6709\u4f9b\u61c9\u5546",gmbPickup:"Google Order \u81ea\u53d6\u4f9b\u61c9\u5546",gmbDelivery:"Google Order \u5916\u9001\u4f9b\u61c9\u5546",gmbPanel:"Google Order \u9762\u677f",noPanel:"\u5df2\u9010\u5e97\u78ba\u8a8d GMB\uff1b\u672a\u898b Google Order \u85cd\u8272\u5165\u53e3",gmbFoundStores:"GMB \u627e\u5230\u9580\u5e02",noLink:"\u672a\u898b\u9023\u7d50",pickup:"\u81ea\u53d6",delivery:"\u5916\u9001",unknown:"\u672a\u77e5",stores:"\u5bb6\u9580\u5e02"};
...
function render(){document.querySelectorAll("#regionFilters button").forEach(button=>button.classList.toggle("active",button.dataset.region===state.region));const cities=[...new Set(stores.filter(store=>state.region===T.all||store.regionGroup===state.region).map(store=>store.city).filter(Boolean))].sort();byId("cityFilter").innerHTML=`<option value="all">${T.allCities}</option>${cities.map(city=>`<option ${state.city===city?"selected":""}>${city}</option>`).join("")}`;const systems=[...new Set(stores.flatMap(store=>[...allSourceClaims(store).map(claim=>claim.system),...(store.gmbOrderLinks||[]).map(link=>link.platform)]).filter(Boolean))].sort();byId("systemFilter").innerHTML=`<option value="all">${T.allSystems}</option>${systems.map(system=>`<option ${state.system===system?"selected":""}>${system}</option>`).join("")}`;const rows=filteredRows();renderInsights(rows);renderKpis(rows);renderMap(rows);renderStoreDistribution(rows);renderModeCharts(rows);renderGmbCharts(rows);renderComparison(rows);renderDetails(rows);}
function platformAuditRows(store){const audit=store.platformAudit||{};return Array.isArray(audit)?audit.filter(Boolean):Object.values(audit).filter(Boolean);}
function platformAuditClaims(store){return platformAuditRows(store).filter(row=>row.platform&&row.evidenceUrl&&!/not_found|no_match|error|invalid/i.test(row.status||"")).map(row=>({system:row.platform,sourceType:row.sourceType||"third_party",orderMode:(row.orderMode||[]).length?row.orderMode:["unknown"],evidenceUrl:row.evidenceUrl,label:row.status||row.platform,confidence:"platform_lead"}));}
function allSourceClaims(store){return[...(store.orderingSystems||[]),...platformAuditClaims(store)];}
function countSystems(rows,options={}){const counts=new Map();rows.forEach(store=>{const systems=new Set();const claims=options.sourceType?(store.orderingSystems||[]):allSourceClaims(store);claims.forEach(claim=>{if(options.sourceType&&claim.sourceType!==options.sourceType)return;if(options.mode&&!(claim.orderMode||[]).includes(options.mode))return;if(claim.system)systems.add(claim.system);});systems.forEach(system=>counts.set(system,(counts.get(system)||0)+1));});return[...counts.entries()].sort((a,b)=>b[1]-a[1]||a[0].localeCompare(b[0]));}
...
function storeHasSystem(store,system){return allSourceClaims(store).some(claim=>claim.system===system)||(store.gmbOrderLinks||[]).some(link=>link.platform===system);}
...
function renderInsights(rows){const total=rows.length||0;const gmbOrdering=rows.filter(store=>storeHasGmbProvider(store)).length;const gmbGap=rows.filter(store=>!storeHasGoogleOrderEntry(store)).length;const anyOrdering=rows.filter(store=>allSourceClaims(store).length).length;const topCity=[...rows.reduce((map,store)=>map.set(store.city,(map.get(store.city)||0)+1),new Map()).entries()].sort((a,b)=>b[1]-a[1])[0]||["-",0];const gmbDelivery=countGoogleOrderOptions(rows,{mode:"delivery"});const leadingDelivery=gmbDelivery[0]||["-",0];byId("insightStrip").innerHTML=`<article><span>\u9580\u5e02\u898f\u6a21</span><strong>${total}</strong><p>${topCity[0]} ${topCity[1]} \u5bb6\u6700\u96c6\u4e2d</p></article><article><span>\u5168\u4f86\u6e90\u5c0e\u5165</span><strong>${total?pct(anyOrdering/total):"0%"}</strong><p>${anyOrdering}/${total} \u5bb6\u6709\u4efb\u4e00\u9ede\u9910\u8b49\u64da</p></article><article><span>Google Order \u4f9b\u61c9\u5546\u8986\u84cb</span><strong>${total?pct(gmbOrdering/total):"0%"}</strong><p>${gmbOrdering} \u5bb6\u6709 Google Order \u4f9b\u61c9\u5546\u8b49\u64da</p></article><article class="warn"><span>\u672a\u627e\u5230 Google Order \u5165\u53e3</span><strong>${gmbGap}</strong><p>\u5df2\u9010\u5e97\u78ba\u8a8d GMB\uff1b\u672a\u898b\u85cd\u8272\u9ede\u9910\u5165\u53e3</p></article><article><span>Google Order \u4e3b\u8981\u4f9b\u61c9\u5546/\u9023\u7d50</span><strong>${systemBadge(leadingDelivery[0])}</strong><p>${leadingDelivery[1]} \u5bb6\u9580\u5e02</p></article>`;}
...
function renderModeCharts(rows){byId("pickupBars").innerHTML=bars(countSystems(rows,{mode:"pickup"}),"",true);byId("deliveryBars").innerHTML=bars(countSystems(rows,{mode:"delivery"}),"alt",true);const regionRows=regions.filter(region=>region!==T.all).map(region=>{const scoped=rows.filter(store=>store.regionGroup===region);const adopted=scoped.filter(store=>allSourceClaims(store).length).length;return[region,scoped.length?adopted/scoped.length:0,`${adopted}/${scoped.length}`];});byId("regionMatrix").innerHTML=regionRows.map(([region,rate,label])=>`<div class="matrix-row"><span>${region}</span><div class="matrix-track"><div class="matrix-fill" style="width:${rate*100}%"></div></div><b>${label}</b></div>`).join("");}
...
function systemKey(system){const value=(system||"").toLowerCase().replace(/\s+/g,"");if(value.includes("eathere")||value.includes("食在麻吉"))return"eathere";if(value==="nidin"||value.includes("nidin"))return"nidin";if(value==="ubereats"||value.includes("ubereats"))return"uber-eats";if(value==="foodpanda"||value.includes("foodpanda"))return"foodpanda";if(value==="pborder"||value.includes("pborder")||value.includes("esgpb"))return"pb-order";if(value==="line"||value.includes("line"))return"line";if(value==="instagram"||value.includes("instagram"))return"instagram";if(value==="quickclick"||value.includes("quickclick")||value.includes("快一點"))return"quickclick";return"other";}
...
function renderDetails(rows){
  ...
  byId("storeRows").innerHTML=rows.map(store=>{
    const allSummary=modeSummary(allSourceClaims(store));
    ...
    const links=allSourceClaims(store).filter(claim=>claim.evidenceUrl).slice(0,8).map(claim=>{const modes=(claim.orderMode||[]).map(mode=>mode==="pickup"?T.pickup:mode==="delivery"?T.delivery:mode).join("/");const label=claim.sourceType==="gmb"?`Google panel ${modes}`:(claim.label||claim.system);return`<a href="${claim.evidenceUrl}" target="_blank">${label}</a>`;}).join(" | ");
    ...
    const orderLinkBlock=orderLinks?`<div class="order-link-list"><b>Google Order 面板連結</b>${orderLinks}</div>`:"";
